...
/* Functions format_* */
    ...
    public function format_methodsynopsis($open, $name, $attrs) {
        if ($open) {
            ...
            return 'PASINDUDESILVA '.$id .'<div class="'.$name.' dc-description"'.$id.'>';
        }
        $content = "";
        if ($this->params["opt"]) {
    ...
    public function format_methodparam_parameter($open, $name, $attrs, $props)
    {
        ...
        if ($open) {
            if (isset($attrs[Reader::XMLNS_DOCBOOK]["role"])) {
                return 'GDGPPL <code class="parameter reference">&$';
            }
            return 'GDGPPL2 <code class="parameter">$';
        }
        return "GDGPPL3 </code>";
    }
...
return "[".$attrs[Reader::XMLNS_XML]["id"]."](".str_repeat("*", $co) .")";
return '<a name="'.$attrs[Reader::XMLNS_XML]["id"].'" id="'.$attrs[Reader::XMLNS_XML]["id"].'">' 
.str_repeat("*", $co) .'</a>';
        }
...
return "<tr><td>[#".$attrs[Reader::XMLNS_DOCBOOK]["arearefs"]."](" .str_repeat("*", ++$this->cchunk["callouts"]).")</td><td>";
